...
#define DEFAULT_LEVLE_LAG -1
#define DEFAULT_SAMPLE_GAM 0
#define DEFAULT_SAMPLE_GEM 0
...
GibbsState::GibbsState()
	: score_(0.0),
	  ...
	  level_lag_(DEFAULT_LEVLE_LAG),
	  sample_eta_(DEFAULT_SAMPLE_ETA),
	  sample_gem_(DEFAULT_SAMPLE_GEM),
...
void GibbsSampler::InitGibbsState(
  ...
  for (int i = 0; i < corpus->getDocuments(); i++) {
  	...
  	// Permute the words in the current documet.
  	...
  	for (int j = depth - 2; j >= 0; j--) {
  		...
  		docuemnt->setPathTopic(j, parent);
  	}
  	...
  	// Sample the document path starting at level 0 
  	// and removing words from levels.
  	if (i > 0) {
  		DocumentTreeUtils::SampleDocumentPath(tree, document, ture, 0);
  	}

  	// Sample levels for this document, and permute the 
...
GibbaState* GibbsSampler::InitGibbsStateRep(
	const std::string& filename_corpus,
	const std::string& filename_settings,
	long random_seed) {
...
void GibbsSampler::IterateGibbsState(GibbsState* gibbs_state) {
	...
	int current iteration = gibbs_state->getIteration();
	...
	// Compute the Gibbs score with the new parrameter values.
	double gibbs_score = gibbs_state->computeGibbsScore();
